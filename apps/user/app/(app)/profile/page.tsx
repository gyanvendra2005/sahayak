 ...
 type Course = {
  ...
  const fetchUserCourses = async () => {
    try {
      ...
      // console.log("Fetched courses:", response.data.data.enrolledCourses);
      
      setCourse(response.data.data.enrolledCourses);
    } catch (error) {
...
console.log(course);
  ...
  // Call fetchUserCourses when component mounts
  useEffect(() => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-0 my-20">
      <h1 className="font-bold text-2xl text-center md:text-left mb-6">
        👤 Profile
      </h1>
      {/* Profile Section */}
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <div className="text-center md:text-left space-y-3">
          <div className="text-lg text-gray-700 dark:text-gray-300">
            ...
            {session?.user.name ?? "Gyanvendra"}
          </div>
          <div className="text-lg text-gray-700 dark:text-gray-300">
            <span className="font-semibold">Email:</span>{" "}
            {session?.user.email ?? "gyan@example.com"}
          </div>
          <div className="text-lg text-gray-700 dark:text-gray-300">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center gap-4">
              <Avatar className="h-20 w-20">
                <AvatarImage
                  src={
                    session?.user.photoUrl || "https://github.com/shadcn.png"
                  }
                  alt="Profile"
                />
